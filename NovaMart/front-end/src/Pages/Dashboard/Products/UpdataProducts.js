...
export default function UpdateProducts() {
  ...
  const showApiImages = apiImage.map((el, index) => (
    <div
      key={index}
      className="border-blue-300 border py-5 px-5 rounded-sm mt-5 w-[335px] flex gap-0 flex-wrap"
    >
      <div className="w-full flex justify-between">
        <div className="flex gap-4">
          ...
          <IoCloseOutline
            onClick={() => handleImageDeleteAPI(el.id)}
            size={30}
            color="#D32107"
  ...
  async function handleImageDeleteAPI(id) {
    setApiImage((prev) => prev.filter((img) => img.id !== id))
  ...
  async function handleUpdate(e) {
    e.preventDefault();
    setLoader(true);
    if(apiImage.length === 0 && images.length === 0) {
        setLoader(false)
        setImageError(true)
    }
    else {
        ...
        <div className="grid gap-2">
          <div className="flex items-center justify-center flex-col">
            <label>
              ...
              />
              <div
                className={`flex w-28 h-9 px-2 flex-col bg-blue-500 
